home_animation = (function() {
	...
	function frame1()
	{
		...
		//wire slides in	
		TweenLite.to(wire, 0.5, {left:0, delay:0});
		//headphone moves left while rotating
		//headphones rotate
		TweenLite.to(black_headphone_1, 1.5, {left:150,rotationY:180, delay:1.2, ease: Expo.easeOut});
		TweenLite.to(black_headphone_blur_left, .4, {left:-50,opacity:1, delay:1.3, ease: Expo.easeOut});	

		//blur image fades in
		TweenLite.to(black_headphone_blur_right, .4, {left:-10,opacity:1, delay:1.3, ease: Expo.easeOut});	
		...
		TweenLite.to(black_headphone_blur_left, .7, {opacity:0, delay:1.4, ease: Expo.easeOut});
		//headphones in final position
		TweenLite.to(black_headphone_left, 1.2, {opacity:1, delay:1.3});	
		TweenLite.to(black_headphone_1, 0, {opacity:0, delay:1.4});	
...
closeBtnEx.onclick = function() {
	...
	document.getElementById("richMedia").innerHTML ='<iframe width="600" height="345" id="player" src="https://www.youtube.com/embed/xata21mVzLs" frameborder="0" allowfullscreen></iframe>';
